...
Run \texttt{TAopt} with an random initial solution.
<<>>=
...
algo <- list(x0 = sol0, neighbour = neighbourU,
             nS = 10000L, nT = 10L, nD = 2000L,
             q = 0.99,
             printBar = FALSE, printDetail = 10000L)
res <- TAopt(OF,algo,Data)
res$OFvalue
@

\textsc{ta} needs quite some time, but it reaches a lower minimum than
the benchmark.
...
restarts <- restartOpt(TAopt, n = 50, OF=OF, algo=algo, Data = Data, 
                       method = "snow", cl = 4)
@ 
