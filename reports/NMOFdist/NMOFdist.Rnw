...
\texttt{parallel} (before, it was \texttt{multicore};
\citealp{Urbanek2011}) or \texttt{snow} \citep{Tierney2011}. This
report mainly serves as a convenient check for these functions. I am
grateful for comments and corrections.

The latest version of the package is available from
\url{http://enricoschumann.net/R/packages/NMOF/index.htm}; the package
...
We also attach a number of other packages. Note that we still use
package \texttt{snow} explicitly. 
...
We also attach a package that helps to generate random numbers for
...
Note that we set up the cluster for \texttt{snow} before we measure
the computing time.
...
With \texttt{snow}, it can help to copy constant function arguments to
the nodes.

Suppose we have a Least-Squares objective function.
<<>>=
OF <- function(b, X, y) {
    temp <- X %*% b - y
    sum(temp^2)
...
\texttt{X} and \texttt{y} stay fixed. We create some random
\texttt{X}, \texttt{y} and \texttt{b}.
<<>>=
ncol <- 10; nrow <- 200
...
Suppose we used such a function in a multiple-solution heuristic like
DE.  We assume the population \texttt{lP} has size \texttt{n}.
<<>>=
...
Next we define various expressions (note that the \texttt{\{\}} are
only necessary for the last one).
<<>>=
...
used, for instance, in a Value-at-Risk calculation, or some kind of
robust statistic).  The function partially sorts a vector \texttt{x}.
<<>>=
fun <- function (x, h)
    sort(x, partial = h)[h]
@
...
that comes with packages \texttt{parallel} and \texttt{snow}; we will
use \texttt{snow} here.
...
This could more have been written more compactly as \texttt{split(P,
  col(P))}; but the simple loop version is faster.  So, we can compare
...
\texttt{parallel} to use just one core (ie, we should gain no
speedup).
<<>>=
...
To pass optional arguments to \texttt{parellel}'s \texttt{mclapply},
we need to collect them in a list \texttt{mc.control}, which needs to
...
\noindent \texttt{TODO: this package is not installed on this machine}
<<eval=false>>=
cl <- makeCluster(c(rep("localhost", nC)), type = "SOCK")
