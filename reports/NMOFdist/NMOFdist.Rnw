...
\nmof\ package \citep*{Gilli2011b}.  Such computations are supported in
the functions \texttt{bracketing}, \texttt{GAopt},
\texttt{gridSearch}, \texttt{restartOpt} through package
\texttt{parallel} (before, it was \texttt{multicore}
\citealp{Urbanek2011}) and \texttt{snow} \citep{Tierney2011}). This
report mainly serves as a convenient collection of test cases for these functions. I am
...
We attach package \textsf{parallel}; we will use package
...
benchmark(lapply(sq, testFun, delay),                ## serial
          mclapply(sq, testFun, delay),              ## package 'multicore'
...
With \texttt{clusterApply}, it can help to copy constant function
