...
<<echo=false>>=
options(continue = "  ", digits = 5, width = 65)
@
\begin{document}
...
\noindent Style analysis, described in
\cite{sharpe1992}, uses a linear factor model to
describe the returns of a portfolio, with a few
constraints added: the factors should be actual asset
classes, typically represented by indices; the factor
loadings, which are interpreted as weights, should sum
to 100\%, and should take on reasonable values.  For a
typical mutual fund, for instance, the weights should
be nonnegative.
...
holds the return observations of the asset classes
(indices): rows are observations; columns are
indices. The vector~$y$ holds the returns of the
portfolio.  All information is collected in a list
\texttt{Data}.
...
We solve the model with Differential Evolution (DE).
...
columns. Or simpler, we might add a column of ones to~$X$,
which is always included as a regressor. In this way,
the residuals will have a zero mean and Least Squares
is equivalent to minimising variance. (But DE does not
...
machine precision allows by increasing
\texttt{settings\$nG}).

<<>>=
## for Least Squares, fit 'Data$y ~ -1 + Data$X' instead
summary(abs(coef(lm(Data$y ~ Data$X))[-1L] - sol$xbest))
@
...
included in the model. We handle these constraints with
a repair function.
<<repair>>=
repair <- function(p, Data) {
  ...
  ## reduce cardinality of solutions (if necessary)
  tmp <- p > 1e-8
  cs <- colSums(tmp)
  ...
  ## remove zero cardinality (if necessary)
  for (i in which(cs == 0))
    p[sample(Data$ni, 1), i] <- 1
...
See also \citet[p.~391]{Gilli2011b}.
<<qp>>=
if (!require("quadprog")) {
  ...
  settings <- list(min = rep(0, Data$ni),
                   ...
                   printBar = FALSE)
  Data$maxi <- Data$ni   ## no cardinality constraint
  sol <- DEopt(ofun, algo = settings, Data = Data)
...
model that a QP solver can handle.


\subsection{No QP needed, example 1: minimising variance}

We stay with the regression model, but now we wish to
minimise the variance of the residuals. We regard every
...
if (!require("quadprog")) {
  ...
  print(unname(coef(lm(y ~ 0 + X))))
  ## variant 2 -- quadprog
  Dmat <- crossprod(X)
  dvec <- y %*% X
  Amat <- as.matrix(rep(0, na))
  message("QP")
  print(solve.QP(Dmat = Dmat, dvec = dvec, Amat = Amat)$solution)
