...
\citealp{Gilli2011b}) is about tools for computational finance,
...
\url{http://enricoschumann.net/NMOF.htm}\medskip
...
\noindent To install the package from an R session, type:
<<eval=false>>=
...
\noindent The source code is also available in the GitHub repository
