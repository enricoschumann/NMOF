...
\citealp{Gilli2011b}) is all about tools for computational finance,
...
(\texttt{{LSopt}}), Particle Swarm (\texttt{{PSopt}}), Threshold
Accepting (\texttt{{TAopt}}) and grid search (\texttt{gridSearch}).
...
\texttt{{callCF}}.  For bond pricing, see \texttt{vanillaBond}.
...
\noindent The book website: \url{http://nmof.net} \smallskip

\noindent The package website:
...
New package releases and other news related to the book or the package
are announced on the \texttt{NMOF-news} mailing list:

\noindent \url{https://lists.r-forge.r-project.org/cgi-bin/mailman/listinfo/nmof-news}

\medskip

\noindent Applications, as long as they are finance-related, should be
discussed on the \texttt{R-SIG-Finance} mailing list:
