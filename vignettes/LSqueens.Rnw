...
queen may move vertically, horizontally and a
...
We may also wanto to visualise a position, for which
may use the function \texttt{print\_board}.
<<>>=
...
Note that for reverse diagonals, the \texttt{N + 1}
would not necessary; it serves to only to shift the
diagonal labels so that the main diagonal is zero.

So for a given solution \texttt{p}, we know the row,
...
A given by position may be modified by picking one row
randomly and then moving the queen there to the left or
right. We allow for moves up to three \texttt{step}
squares.
...
We use three LS method: a `classical' local search,
