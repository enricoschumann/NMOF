...
the code to build the matrix is in the source files of
this vignette.
...
\noindent The function
\texttt{mvPortfolio}\marginpar{\texttt{mvPortfolio}}
...
\citet{rockafellar2000}.  See \emph{Minimising
  Conditional Value-at-Risk (CVaR)} (\url{Minimising
  Conditional Value-at-Risk}) for more details
