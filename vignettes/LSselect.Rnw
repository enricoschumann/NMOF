...
problem; for more details, please see \citet[ch. 12
and~13]{Gilli2011b}.  We start by attaching the package and fixing a
seed.
...
C <- array(0.6, dim = c(na, na)) ## the correlation matrix
diag(C) <- 1

minVol <- 0.20; maxVol <- 0.40
Vols <- (maxVol - minVol) * runif(na) + minVol
Sigma <- outer(Vols, Vols) * C   ## the covariance matrix
@
...
With this implementation we assume that \texttt{Data\$Kmax >
  Data\$Kmin}. (If \texttt{Data\$Kmax == Data\$Kmin}, then
...
par(bty = "n")
plot(ecdf(allResOF), main = "Portfolio volatility", pch = 21)
@
(We run \texttt{LSopt} only \Sexpr{nRuns} times to keep the build time for the
