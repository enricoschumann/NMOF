...
The variable \texttt{nRuns} sets the number of restarts for the
examples to come. It is set to only two here to keep the build-time of
the package acceptable; feel free to increase it.
<<>>=
nRuns <- 2
@
...
We start by attaching the package and creating a `true' yield curve
\texttt{yM} with given parameters \texttt{betaTRUE}.  The
times-to-payment, measured in years, are collected in the vector
\texttt{tm}. We set a seed to make the computations reproducible.
