...
\maketitle
\begin{abstract}
  \noindent A brief tutorial on how to use Differential Evolution (DE)
  to fit the Nelson--Siegel model.
\end{abstract}
...
We start by attaching the package; the variable \texttt{nRuns} sets
the number of restarts for the examples to come. It is set to only two
here to keep the build-time of the package acceptable; feel free to
increase it. We set a seed to make the computations reproducible.
<<>>=
require("NMOF")
nRuns <- 2L
set.seed(112233)
