...
the code is this vignette uses the scripts \texttt{exampleSquaredRets.R}, \texttt{exampleSquaredRets2.R} and \texttt{exampleRatio.R}.
...
\section{Minimising a risk--reward ratio}
to be added
