...
cat("lqs:   ", res1, "\n",
    ...
    "DEopt: ", res3, "\n", sep = "")
@
To demonstrate the advantage of a vectorised objective function, we can compare it with looping over the solutions.
<<>>=
...
system.time(solDE <- DEopt(OF = OF, algo = de, data = data))
@
Finally, we can show the effect that DE performs better with a small value for \texttt{F}. But as so often: 
increasing the computational resources (eg, increasing generations) helps.
