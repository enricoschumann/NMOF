...
\title{Robust Regression with
  Particle Swarm Optimisation and Differential Evolution}
\author{Enrico Schumann} \usepackage[noae]{Sweave}
\usepackage{mathptmx} \usepackage{natbib} \usepackage{amsmath,amstext}
\begin{document}
\maketitle
\begin{abstract}
  \noindent A brief tutorial on using Differential Evolution and
  Particle Swarm Optimisation to estimate a regression model.
\end{abstract}
...
t1de[[3L]]/t2de[[3L]]
@
Finally, we can show the effect that DE performs better with a small
value for \texttt{F}. (We run only three restarts to keep the time to
build the vignette acceptable. When redoing this analysis, you should
increase \texttt{trials}.)
...
for (i in seq_len(trials))
    allResF02[i] <- resF02[[i]]$OFvalue
for (i in seq_len(trials))
    allResF08[i] <- resF08[[i]]$OFvalue
...
But as so often: increasing the computational resources (eg,
increasing generations) helps. (Check whether the populations have
converged.)
