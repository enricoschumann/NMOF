...
test.DEopt <- function() {
	...
	# check if Fmat/Plist are returned
    # ...if FALSE	
	trefethen <- function(xx) {
		x <- xx[1L]; y <- xx[2L]
		res <- exp(sin(50*x)) + sin(60*exp(y)) + sin(70*sin(x)) +
	...
	checkEquals(length(sol$Plist), algo$nG)
	checkEquals(dim(sol$Plist[[algo$nG]]), c(length(algo$min), algo$nP))
}
